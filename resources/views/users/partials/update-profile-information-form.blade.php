<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ "Вы можете обновить информацию вашей учетной записи." }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('users.update', ['user' => Auth::user()]) }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')

        <div>
            <x-inputs.input-label for="name" value="{{ 'Имя' }}" />
        ...
        <div>
            <x-inputs.input-label for="email" value="{{'Email'}}" />
